  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-black text-white p-6 rounded-xl shadow-lg relative overflow-hidden">
        ...
        <p className="text-xs text-gray-400 font-bold tracking-widest uppercase mb-1">{t('statusApproved')} / {t('statusPaid')}</p>
        <h2 className="text-4xl font-bold">${totalApproved.toLocaleString('en-US', { minimumFractionDigits: 2 })}</h2>
        <p className="text-xs text-gray-200 mt-2">{approvedQuotes.length} {t('dashCount')}</p>
      </div>

      {/* CARD DE PENDENTES (DINHEIRO POTENCIAL) */}
      <div className="bg-white border border-gray-200 text-gray-800 p-6 rounded-xl shadow-lg relative overflow-hidden">
        <p className="text-xs text-gray-400 font-bold tracking-widest uppercase mb-1">{t('statusPending')}</p>
        <h2 className="text-3xl font-bold">${totalPending.toLocaleString('en-US', { minimumFractionDigits: 2 })}</h2>
        <p className="text-xs text-gray-500 mt-2">{pendingQuotes.length} {t('dashCount')}</p>
      </div>
